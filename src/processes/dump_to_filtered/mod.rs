use crate::{
    constants::DELIMITER, traits::Seekable, utils::redact_comment_block::redact_comment_block,
};
use std::{
    fs::File,
...
/// search_headers can be any language's "==Russian==" header
pub fn dump_to_filtered(dump: &str, out: &str, search_headers: &[&str]) -> Result<(), String> {
    ...
    loop {
        let bytes_read = reader.read(&mut active_buffer).expect("read from file");
        ...
        for page_start_pos in page_start_tag_occurrences {
            if let Some(page_end_pos) = &active_buffer[page_start_pos..].as_ref().find(PAGE_END_TAG)
            {
                    ...
                    .any(|h| page_content.as_ref().find(h.as_bytes()).is_some())
                {
                    ...
                    out_buffer.write(DELIMITER.as_bytes()).expect("File write");
